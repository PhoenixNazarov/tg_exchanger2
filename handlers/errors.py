...
@router.errors()
async def error_handle(update: Update, exception):
    print('error_handle', exception)
    callback_query = None
    ...
    elif isinstance(exception, ChangeMessage):
        await message.edit_text(exception.new_text)

    elif isinstance(exception, MyTgException):
        print('MyTgException')
